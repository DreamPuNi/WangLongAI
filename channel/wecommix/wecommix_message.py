...
class WecomMixMessage(ChatMessage):
    def __init__(self, wework_msg, wework, is_group=False):
        ...
        """
        try:
            super().__init__(wework_msg) # 先让ChatMessage初始化wework_msg,然后再传递给WeworkMessage
            ...
            logger.error(f"初始化ChatMessage失败: {e}")
            raise e